...
exports.seed = function (knex)
{
    ...
    return knex.schema.withSchema('rcg_tms')
        .raw(`TRUNCATE TABLE rcg_tms.${terminalTableName}, rcg_tms.${contactsTableName} CASCADE`)
        .then(function ()
        {
            const terminals = [];
            for (let i = 0; i < 30; i++)
            ...
            return knex(terminalTableName).insert(terminals).onConflict(['latitude', 'longitude'], ['name']).ignore();
        });
};
